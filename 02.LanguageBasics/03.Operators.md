...
* **MSB (leftmost bit) = 0** → The number is  **positive** .
  * Just read the binary as usual.
  * Example: `0111` = 7.
* **MSB (leftmost bit) = 1** → The number is  **negative** .
  * Apply **two’s complement once** (flip bits + add 1) to find the magnitude.
  * Then put the negative sign.
  * Example: `1011` → flip → `0100` → add 1 → `0101` = 5 → result =  **-5** .

## 🔹 Why This Works

* Two’s complement encoding is designed so that arithmetic works seamlessly.
* The MSB acts as the  **sign indicator** :
  * `0` → safe zone (positive).
  * `1` → negative zone (decode with two’s complement).

So what is that??

## Unsigned Binary

* All bits represent magnitude (no sign).
...
* MSB (leftmost bit) = sign indicator:

  * `0` → positive → read normally.
  * `1` → negative → apply two’s complement to find magnitude.
...
* Example:
  ...
  ```
  0101 (5) → 1010 (represents -6 in 4‑bit two’s complement)
  ```
...
* Example:
  ...
  ```
  0101 (5) → flip → 1010 → add 1 → 1011 = -5
  ```

## 5. Bitwise NOT (~)

* Flips all bits (same as 1’s complement).
* Interpretation depends on MSB:
  * If MSB = 0 → positive → read directly.
  * If MSB = 1 → negative → apply two’s complement once.
* Formula:

~ x =  (-x) -1

Example :-  We have  5 to find ~5
...
Now the question is: how do we interpret `1010`?

cause here in the left most bit is which is negative so we have to implement 2's compliment

## 🔹 Step 3: Interpretation in Two’s Complement

* Since MSB = 1, this is a **negative number** in two’s complement.
* To find its value:
* 1. Flip bits → `0101`
  2. Add 1 → `0110` = 6
  3. Apply negative sign → **-6**

So:

```
# so here we have 1010 now we have to do 2's compliment
here flip the bits 
0101
# add 1 to it
...
now apply - cause it is negative
...
For Example if we get 0 in left most msb then we don't apply any 2's complement here

```
we have 1000 which is 8

~8 is 0111

so here left most msb is 0 which is positive  
so we read it as it is

```

* If the result of `~` has  **MSB = 0** , it’s positive → no need for two’s complement conversion.
* If the result has  **MSB = 1** , then it’s negative → apply two’s complement once to find its magnitude.
...
1's complement is found by inverting all the bits of a binary number (0s become 1s and 1s become 0s). 2's complement is found by first taking the 1's complement and then adding 1 to the result

# 1's Complement

* **How to find it:** Flip every bit in the binary number.
* **Example:** For the binary number  1010

  1's Complement : 0101
* Disadvantages: it creates two representations for zero
* Two representations for zero:

  * `0000` → +0
  * `1111` → -0
* This causes confusion and extra logic in arithmetic.
...
* **Example:** For the binary number  1010
  ...
  1. Step1: find the 1's complement for 1010 which is 0101.
  2. Step 2: add 1
     0101 ->
* **Advantage:** It has only one representation for zero and is used by most modern computers to perform binary arithmetic with both positive and negative integers.** **
...
here b is no of bits and we get answer  a multiply by 2 power of b times
...
Operator precedence in C++ determines which operators are evaluated first in an expression. Higher‑precedence operators are executed before lower‑precedence ones, and associativity decides the direction (left‑to‑right or right‑to‑left) when operators share the same precedence.

just like BODMAS rule in Maths


![1765445879185](image/03.Operators/1765445879185.png)
...
* **Left-to-right associativity:** means that when multiple operators of the same precedence appear in an expression, they are evaluated from left to right.
  * For example, in the expression a + b - c, addition and subtraction have the same precedence and are left-associative,
  *  so the expression is evaluated as (a + b) - c.
* ****Right-to-left associativity:**** means the operators are evaluated from right to left.
  * For example, the assignment operator = is right-associative. So, in the expression a = b = 4, the value 4 is first assigned to b, and then the result of that assignment (b, which is now 4) is assigned to a.




![1765445912748](image/03.Operators/1765445912748.png)
...
![1765445928512](image/03.Operators/1765445928512.png)
